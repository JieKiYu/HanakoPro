...
describe('InputArea layout', () => {
  it('keeps chat, composer, welcome, and bridge widths in their intended lanes', () => {
    ...
    expect(messageAssistantBlock).toMatch(/--chat-code-block-width:\s*calc\(100% - var\(--chat-message-action-safe-area\)\)/);
    ...
    expect(assistantMarkdownBlock).toMatch(/width:\s*100%/);
    expect(assistantMarkdownBlock).toMatch(/max-width:\s*100%/);
