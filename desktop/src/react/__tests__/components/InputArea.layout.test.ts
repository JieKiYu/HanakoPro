...
describe('InputArea layout', () => {
  it('keeps chat, composer, welcome, and bridge widths in their intended lanes', () => {
    ...
    const goalAcceptanceWrapperBlock = cssBlock(chatCss, String.raw`\.moodWrapper\[data-yuan="goal-acceptance"\]`);
    const goalAcceptanceLabelBlock = cssBlock(chatCss, String.raw`\.goalAcceptanceLabel`);
    const goalAcceptanceMarkBlock = cssBlock(chatCss, String.raw`\.goalAcceptanceMark`);
    ...
    const goalAcceptanceConclusionBlock = cssBlock(chatCss, String.raw`\.goalAcceptanceConclusion`);
    const goalAcceptanceConclusionBodyBlock = cssBlock(chatCss, String.raw`\.goalAcceptanceConclusionBody`);
    const goalAcceptanceConclusionUsageBlock = cssBlock(chatCss, String.raw`\.goalAcceptanceConclusionUsage`);
    const thinkingBlock = cssBlock(chatCss, String.raw`\.thinkingBlock`);
    const thinkingBlockBody = cssBlock(chatCss, String.raw`\.thinkingBlockBody`);
    ...
    expect(goalAcceptanceWrapperBlock).toMatch(/--mood-psych-color:\s*#174EA6/);
    expect(goalAcceptanceWrapperBlock).toMatch(/--mood-psych-bg:\s*rgba\(23,\s*78,\s*166,\s*0\.075\)/);
    expect(goalAcceptanceWrapperBlock).toMatch(/--mood-psych-border:\s*rgba\(23,\s*78,\s*166,\s*0\.34\)/);
    expect(goalAcceptanceLabelBlock).toMatch(/display:\s*inline-flex/);
    expect(goalAcceptanceMarkBlock).toMatch(/color:\s*var\(--mood-psych-color\)/);
    expect(goalAcceptanceConclusionBlock).toMatch(/width:\s*var\(--chat-code-block-width,\s*100%\)/);
    expect(goalAcceptanceConclusionBlock).toMatch(/max-width:\s*100%/);
    expect(goalAcceptanceConclusionBlock).toMatch(/min-width:\s*0/);
    expect(goalAcceptanceConclusionBlock).toMatch(/border:\s*1px solid rgba\(23,\s*78,\s*166,\s*0\.28\)/);
    expect(goalAcceptanceConclusionBodyBlock).not.toMatch(/border-left/);
    expect(goalAcceptanceConclusionBodyBlock).not.toMatch(/margin-left/);
    expect(goalAcceptanceConclusionBodyBlock).toMatch(/overflow-wrap:\s*anywhere/);
    expect(goalAcceptanceConclusionUsageBlock).toMatch(/font-family:\s*var\(--font-mono\)/);
    expect(thinkingBlock).toMatch(/width:\s*var\(--chat-code-block-width,\s*100%\)/);
    ...
    expect(thinkingBlockBody).toMatch(/width:\s*100%/);
    expect(thinkingBlockBody).toMatch(/max-width:\s*100%/);
    expect(thinkingBlockBody).toMatch(/box-sizing:\s*border-box/);
    expect(thinkingBlockBody).toMatch(/overflow-wrap:\s*anywhere/);
  ...
  it('keeps new goals in the composer and existing goals in the active goal bar', () => {
    ...
    expect(inputAreaSource).not.toMatch(/goalStartPrompt|goalUpdatePrompt|goalResumePrompt/);
    expect(inputAreaSource).not.toMatch(/buildGoalStartPrompt|buildGoalUpdatePrompt|buildGoalResumePrompt/);
    expect(inputAreaSource).toMatch(/buildGoalPromptMessage\(result\.objective\)/);
    ...
    expect(inputAreaSource).toMatch(/sendPlainPrompt\(message\.promptText, message\.displayText/);
