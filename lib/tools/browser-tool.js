...
const BROWSER_START_NAVIGATION_NOTE_ZH = "浏览器已启动，但还没有打开具体页面。若任务需要画布、页面、仪表盘、预览或 dev server，请直接找到或启动对应服务并调用 browser.navigate 打开真实 URL；无需再用正文重复说明同一入口。";
const BROWSER_START_NAVIGATION_NOTE_EN = "Browser is running, but no specific page has been opened yet. If the task needs a canvas, page, dashboard, preview, or dev server, directly find or start the service and call browser.navigate for the real URL; do not repeat the same entrypoint in prose.";
...
export function createBrowserTool(getSessionPath, options = {}) {
  ...
  function browserStartResultText(baseText) {
    const note = t("error.browserStartNeedsNavigation");
    const resolvedNote = note === "error.browserStartNeedsNavigation"
      ? BROWSER_START_NAVIGATION_NOTE_ZH
      : note;
    return `${baseText}\n\n${resolvedNote}`;
  }

  function browserStartDetails(fields, status) {
    return {
      status,
      needsNavigation: true,
      navigationHint: BROWSER_START_NAVIGATION_NOTE_ZH,
      navigationHintEn: BROWSER_START_NAVIGATION_NOTE_EN,
      ...fields,
    };
  }
        ...
        switch (params.action) {
          ...
          case "start": {
            if (browser.isRunning(sessionPath)) {
              ...
              return toolOk(
                browserStartResultText(t("error.browserAlreadyRunning")),
                browserStartDetails(await statusFields(sessionPath), "already_running"),
              );
            }
            ...
            return toolOk(
              browserStartResultText(t("error.browserLaunched")),
              browserStartDetails(await statusFields(sessionPath), "launched"),
            );
          }
