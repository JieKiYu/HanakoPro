...
function buildActionGuidance(snapshot) {
  ...
  const pointActions = ["click_point", "double_click"]
    .filter((action) => (!hasExplicitActions || allowedActions.includes(action)) && isModelUsableCapability(capabilities.pointClick));
  ...
  if ((!hasExplicitActions || allowedActions.includes("drag")) && isModelUsableCapability(capabilities.drag)) {
