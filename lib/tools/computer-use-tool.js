...
function isModelUsableCapability(value) {
  return value === true
    || value === "allowed"
    || value === "semantic"
    || value === "focused"
...
function modelVisibleActionCapabilities(capabilities = {}) {
  ...
  if (capabilities?.elementDoubleClick === true) {
    visible.elementDoubleClick = true;
  }
  if (isModelUsableCapability(capabilities?.pointClick)) {
    visible.pointClick = capabilities.pointClick;
  }
  if (isModelUsableCapability(capabilities?.drag)) {
    visible.drag = capabilities.drag;
  }
  return visible;
}
...
function actionTarget(params) {
  ...
  if (params.x != null || params.y != null) {
    return { coordinateSpace: "screenshot", x: params.x, y: params.y };
  }
  return undefined;
}
...
function scoreWindowTitleMatch(win, requestedTitle) {
  const query = normalizeAppLookup(requestedTitle);
  if (!query) return 0;
  const title = normalizeAppLookup(win?.title || win?.name || "");
  if (!title) return 0;
  if (title === query) return 100;
  if (title.includes(query) || query.includes(title)) return 80;
  const queryTokens = lookupTokens(query);
  return queryTokens.length && queryTokens.every((token) => title.includes(token)) ? 70 : 0;
}

function findWindowIdByTitle(app, requestedTitle) {
  const matches = (Array.isArray(app?.windows) ? app.windows : [])
    .map((win) => ({ win, score: scoreWindowTitleMatch(win, requestedTitle) }))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score);
  if (!matches.length) return null;
  const bestScore = matches[0].score;
  ...
  const windowId = best[0].win?.windowId;
  return windowId ? String(windowId) : null;
}
...
async function resolveStartTarget(toolCtx, host, params) {
  ...
  return {
    ...
    windowId: target.windowId || findWindowIdByTitle(app, target.windowTitle) || firstWindowId(app) || undefined,
  };
}
...
function buildActionGuidance(snapshot) {
  ...
  const capabilities = snapshot.actionCapabilities || {};
  const hasScreenshot = Boolean(snapshot.screenshot?.data);
  ...
  if (elementActions.length) {
    lines.push(`- Use element ids with ${listActionNames(elementActions)}.`);
  }
  const pointActions = ["click_point", "double_click"]
    .filter((action) => hasScreenshot && (!hasExplicitActions || allowedActions.includes(action)) && isModelUsableCapability(capabilities.pointClick));
  if (pointActions.length) {
    lines.push(`- Use screenshot coordinates with ${listActionNames(pointActions)} when the target is visible in the screenshot but has no useful element id. Coordinates are window-local screenshot pixels from this snapshot.`);
  }
  if (hasScreenshot && (!hasExplicitActions || allowedActions.includes("drag")) && isModelUsableCapability(capabilities.drag)) {
    lines.push("- Use drag with fromX/fromY/toX/toY for window-local screenshot pixel drag gestures.");
  }
  if (!lines.length) {
    return "- This provider currently exposes no usable action for the target. Report that the target cannot be clicked by this provider.";
  }
  if (hasScreenshot) {
    lines.push("- Prefer element ids when they clearly match; use screenshot coordinates for visual-only UI or verification clicks.");
  } else {
    lines.push("- No screenshot was returned; continue with the accessibility element ids and do not use screenshot coordinates until a later snapshot includes an image.");
  }
  return lines.join("\n");
}
