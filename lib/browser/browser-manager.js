...
function _localUrlEndpoint(url) {
  let parsed;
  try {
    parsed = new URL(url);
  } catch {
  ...
  return {
    host: host === "localhost" || host === "0.0.0.0" ? "127.0.0.1" : host.replace(/^\[(.*)\]$/, "$1"),
    port,
  };
}

function _canConnectLocalUrl(url, timeoutMs = 450) {
  const endpoint = _localUrlEndpoint(url);
...
export class BrowserManager {
  ...
  async resumeForSession(sessionPath) {
    ...
    if (!savedUrl) return;
    const isLocalColdUrl = !!_localUrlEndpoint(savedUrl);
    const localUrlAlive = await _canConnectLocalUrl(savedUrl);
    if (!localUrlAlive) {
      this._removeColdUrl(sessionPath);
      console.log("[browser] 跳过本地 URL 冷恢复（端口不可达）", savedUrl, sessionPath);
      return;
    }
    ...
    try {
      ...
      if (isLocalColdUrl) {
        ...
        this._removeLru(sessionPath);
        this._removeColdUrl(sessionPath);
        console.log("[browser] 本地 URL 冷恢复导航失败，已清理", savedUrl, sessionPath, _errorMessage(error));
        return;
      }
      // 非本地 URL 维持旧行为：保留 savedUrl，允许用户稍后手动打开或重试。
    }
  }
