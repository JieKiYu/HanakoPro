...
const OPENAI_SDK_FINGERPRINT_HEADER_OVERRIDES = Object.freeze({
  "User-Agent": null,
...
function isAcuiOpenAiCompatibleModel(model) {
  if (!model || typeof model !== "object") return false;
  if (model.api !== "openai-completions" && model.api !== "openai-responses") return false;
  const provider = String(model.provider || "").toLowerCase();
  if (provider === "acui") return true;
  const baseUrl = String(model.baseUrl || model.base_url || "");
  try {
    const host = new URL(baseUrl).hostname.toLowerCase();
    return host === "api.acui.shop" || host.endsWith(".api.acui.shop");
  } catch {
    return baseUrl.toLowerCase().includes("api.acui.shop");
  }
}

export function applyOpenAiSdkHeaderCompat(model, headers) {
  if (!isAcuiOpenAiCompatibleModel(model)) return headers;
  return {
    ...(headers || {}),
    ...OPENAI_SDK_FINGERPRINT_HEADER_OVERRIDES,
  };
}

function wrapModelRegistryForHanaCompat(registry) {
  if (!registry || typeof registry.getApiKeyAndHeaders !== "function") return registry;
  if (registry.__hanaCompatWrapped) return registry;
  const rawGetApiKeyAndHeaders = registry.getApiKeyAndHeaders.bind(registry);
  Object.defineProperty(registry, "getApiKeyAndHeaders", {
    configurable: true,
    value: async (model) => {
      const result = await rawGetApiKeyAndHeaders(model);
      if (!result?.ok) return result;
      return {
        ...result,
        headers: applyOpenAiSdkHeaderCompat(model, result.headers),
      };
    },
  });
  Object.defineProperty(registry, "__hanaCompatWrapped", {
    configurable: true,
    value: true,
  });
  return registry;
...
export async function createAgentSession(options) {
  ...
  const sessionOptions = !options?.agentDir && typeof resourceLoaderAgentDir === "string" && resourceLoaderAgentDir
    ...
    : options;
  const compatOptions = sessionOptions?.modelRegistry
    ? { ...sessionOptions, modelRegistry: wrapModelRegistryForHanaCompat(sessionOptions.modelRegistry) }
    : sessionOptions;
  const result = await rawCreateAgentSession(normalizeCreateAgentSessionOptions(compatOptions));
...
// ── Utilities ──
export { formatSkillsForPrompt, getAgentDir, getLastAssistantUsage } from "@mariozechner/pi-coding-agent";
...
export function createModelRegistry(authStorage, modelsJsonPath) {
  return wrapModelRegistryForHanaCompat(ModelRegistry.create(authStorage, modelsJsonPath));
}
